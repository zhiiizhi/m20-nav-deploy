...
namespace q {
    ...
    private:
        ...
        int JointDataNormalCheck() {
            ...
            Vec3f fl_lower, fl_upper;
            fl_lower << cp_ptr_->fl_joint_lower_;
            fl_upper << cp_ptr_->fl_joint_upper_;
            ...
            for (int i = 0; i < 12; ++i) {
                ...
                if (std::isnan(joint_vel_(i))) {
                    return 4;
                        ...
                        if (joint_vel_(i) > cp_ptr_->joint_vel_limit_(i % 3) + 0.1) return 6;
                }
            }
            return 0;
        }
        ...
        void DisplayProprioceptiveInfo() {
            std::cout << "Joint Data: \n";
            std::cout << "FL pos: ";
            PrintEigenVectorf(joint_pos_.segment(0, 3));
            std::cout << "FR pos: ";
            PrintEigenVectorf(joint_pos_.segment(3, 3));
            std::cout << "HL pos: ";
            PrintEigenVectorf(joint_pos_.segment(6, 3));
            std::cout << "HR pos: ";
            PrintEigenVectorf(joint_pos_.segment(9, 3));
            std::cout << "vel: ";
    ...
    public:
        ...
        virtual void Run() {
            ...
            if (!initial_ts_flag_) {
                initial_ts_flag_ = true;
                initial_ts = ri_ptr_->GetInterfaceTimeStamp();
            }
            double current_ts = ri_ptr_->GetInterfaceTimeStamp();
            if (((current_ts - last_print_time) > 1)) {
                DisplayProprioceptiveInfo();
                DisplayAxisValue();
                // std::cout << "last_print_time: " << (last_print_time - initial_ts) << std::endl;
                // std::cout << "current_ts: " << (current_ts - initial_ts) << std::endl;
                // std::cout << "time_diff: " << (current_ts - last_print_time) << std::endl;
                last_print_time = current_ts;
            }
