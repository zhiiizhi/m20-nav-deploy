...
class Localization {
   public:
    ...
    /// 处理/ODOM消息（直接喂给PGO的DR通道）
    void ProcessOdomMsg(const nav_msgs::msg::Odometry::SharedPtr odom_msg);
    ...
    /// 获取PGO定位结果（正确的地图坐标系位姿）
    const LocalizationResult& GetLocResult() const { return loc_result_; }
    ...
    void LidarLocOnlyProcCloud(CloudPtr);
