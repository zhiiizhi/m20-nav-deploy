...
bool PGO::ProcessLidarLoc(const LocalizationResult& loc_result) {
    ...
    // 如果相对位姿(DR和LidarOdom有一个即可)还没来
    if (RelativePoseQueueEmpty()) {
        // 首个有效 LidarLoc 直接发布为绝对锚点，不要求 LO/DR
        if (loc_result.lidar_loc_valid_ && high_freq_output_func_) {
            static bool first_loc_published = false;
            if (!first_loc_published) {
                first_loc_published = true;
                parking_result_ = loc_result;
                parking_result_.valid_ = true;
                parking_result_.status_ = LocalizationStatus::GOOD;
                LOG(INFO) << "PGO publishing first LidarLoc directly as absolute anchor";
                high_freq_output_func_(parking_result_);
                return true;
            }
        }
