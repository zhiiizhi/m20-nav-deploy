...
bool Localization::Init(const std::string& yaml_path, const std::string& global_map_path) {
    ...
    lidar_odom_proc_cloud_.SetMaxSize(50);
    lidar_loc_proc_cloud_.SetMaxSize(50);
...
void Localization::ProcessLidarMsg(const sensor_msgs::msg::PointCloud2::SharedPtr cloud) {
    UL lock(global_mutex_);
    if (lidar_loc_ == nullptr || pgo_ == nullptr) {
        return;
    }
    ...
    // Send first 10 frames to LIO to bootstrap IMU state for DR
    static int lio_init_count = 0;
    if (++lio_init_count <= 10 && lio_ != nullptr) {
        lidar_odom_proc_cloud_.AddMessage(laser_cloud);
    }

    if (options_.online_mode_) {
        lidar_loc_proc_cloud_.AddMessage(laser_cloud);
    } else {
        LidarLocOnlyProcCloud(laser_cloud);
    }
}
...
void Localization::LidarLocOnlyProcCloud(CloudPtr cloud) {
    // Send raw preprocessed cloud directly to lidar_loc, no LIO
    lidar_loc_->ProcessCloud(cloud);

    auto res = lidar_loc_->GetLocalizationResult();
    pgo_->ProcessLidarLoc(res);

    if (ui_) {
        ui_->UpdateScan(cloud, res.pose_);
    }
...
void Localization::ProcessOdomMsg(const nav_msgs::msg::Odometry::SharedPtr odom_msg) {
    ...
    double this_odom_time = lightning::ToSec(odom_msg->header.stamp);
    ...
    // heartbeat: 每100帧打印一次，确认/ODOM被接收
    static int odom_count = 0;
    if (++odom_count % 100 == 1) {
        LOG(INFO) << "/ODOM received #" << odom_count << " pos="
                  << odom_msg->pose.pose.position.x << ","
                  << odom_msg->pose.pose.position.y << ","
                  << odom_msg->pose.pose.position.z;
    }

    // 直接将/ODOM转换为NavState，喂给PGO的DR通道
    NavState dr_state;
    dr_state.timestamp_ = this_odom_time;
    dr_state.pos_ = Vec3d(odom_msg->pose.pose.position.x,
                          odom_msg->pose.pose.position.y,
                          odom_msg->pose.pose.position.z);
    dr_state.rot_ = SO3(Quatd(odom_msg->pose.pose.orientation.w,
                               odom_msg->pose.pose.orientation.x,
                               odom_msg->pose.pose.orientation.y,
                               odom_msg->pose.pose.orientation.z));
    dr_state.vel_ = Vec3d(odom_msg->twist.twist.linear.x,
                          odom_msg->twist.twist.linear.y,
                          odom_msg->twist.twist.linear.z);
    dr_state.pose_is_ok_ = true;

    // 送给PGO和LidarLoc的DR通道（替代LIO的DR）
    lidar_loc_->ProcessDR(dr_state);
    pgo_->ProcessDR(dr_state);
}
